// Checkup weights and grade bands follow the spec (docs/superpowers/specs, section 5).
...
export const SCREEN_OPTIONS = [
  ...
  { key: "cracked_unusable", label: "Trincada afetando uso", points: 0 },
...
export const BATTERY_OPTIONS = [
  ...
  { key: "below_80", label: "Abaixo de 80% qualquer origem", points: 4 },
] as const satisfies readonly Option<string>[];
...
export const BIOMETRICS_OPTIONS = [
  ...
  { key: "broken", label: "Não funciona", points: 0 },
] as const satisfies readonly Option<string>[];
...
export const BODY_OPTIONS = [
  ...
  { key: "heavy_dent", label: "Amassado grave", points: 0 },
] as const satisfies readonly Option<string>[];
...
export function computeScore(answers: CheckupAnswers) {
  ...
  const penalty = SERVICE_OPTIONS.find((o) => o.key === answers.service)?.penalty ?? 0;
...
export function computePricing(input: PricingInput) {
  ...
  const maxPurchaseCost = resalePrice - input.repairTotal - resalePrice * input.minMargin;
