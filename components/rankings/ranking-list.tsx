...
export function RankingList({ title, items, emptyMessage }: { title: string; items: RankingItem[]; emptyMessage: string }) {
  return (
    <section className="overflow-hidden rounded-xl bg-card shadow-card">
      <h2 className="border-b border-[#242424] px-5 py-4 text-[13px] font-semibold text-foreground">{title}</h2>
      {items.length === 0 ? (
        ...
        <ol>
          {items.map((item, index) => (
            <li
              key={item.key}
              className="flex flex-col gap-1 border-b border-[#242424] bg-[#1A1A1A] px-5 py-[14px] text-[13px] last:border-b-0 md:grid md:grid-cols-3 md:items-center md:gap-4 md:text-sm"
            >
